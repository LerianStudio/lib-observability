...
// Recorder.RecordHistogram when the float64 value cannot be recorded
...
// Recorder mirrors the recording surface of MetricsFactory using
...
//
// Recorder flattens the builder chain into a single call per
...
// factoryRecorder is the Recorder view of a *MetricsFactory.
type factoryRecorder struct {
	factory *MetricsFactory
}

// AsRecorder adapts a MetricsFactory to the universal form described on
// Recorder.
//
// AsRecorder(nil) returns a working no-op recorder rather than a nil
...
func AsRecorder(f *MetricsFactory) Recorder {
	return &factoryRecorder{factory: f}
}
...
func (r *factoryRecorder) AddCounter(ctx context.Context, name, description, unit string, attrs map[string]string, delta int64) error {
...
func (r *factoryRecorder) SetGauge(ctx context.Context, name, description, unit string, attrs map[string]string, value int64) error {
	if r == nil || r.factory == nil {
		return nil
	}
...
// recording — see the precision note on Recorder — and a value that
...
func (r *factoryRecorder) RecordHistogram(ctx context.Context, name, description, unit string, attrs map[string]string, value float64, buckets []float64) error {
	if r == nil || r.factory == nil {
		return nil
	}
