...
// Float64CounterBuilder provides a fluent API for recording float64 counter metrics with optional labels.
// It mirrors CounterBuilder and shares its sentinel errors: the failure is "this builder or its
// instrument is nil", which does not differ by value kind.
type Float64CounterBuilder struct {
	factory *MetricsFactory
...
// WithLabels adds labels/attributes to the counter metric.
// Returns a nil-safe builder if the receiver is nil.
func (c *Float64CounterBuilder) WithLabels(labels map[string]string) *Float64CounterBuilder {
	if c == nil {
		return nil
	}

	builder := &Float64CounterBuilder{
		factory: c.factory,
		counter: c.counter,
		name:    c.name,
		attrs:   make([]attribute.KeyValue, 0, len(c.attrs)+len(labels)),
...
// WithAttributes adds OpenTelemetry attributes to the counter metric.
// Returns a nil-safe builder if the receiver is nil.
func (c *Float64CounterBuilder) WithAttributes(attrs ...attribute.KeyValue) *Float64CounterBuilder {
	if c == nil {
		return nil
	}
...
// Add records a counter increment.
// Returns an error if the value is negative (counters are monotonically increasing).
func (c *Float64CounterBuilder) Add(ctx context.Context, value float64) error {
	if c == nil {
		return ErrNilCounterBuilder
	}
...
// AddOne increments the counter by one.
func (c *Float64CounterBuilder) AddOne(ctx context.Context) error {
	if c == nil {
		return ErrNilCounterBuilder
	}
...
// Float64GaugeBuilder provides a fluent API for recording float64 gauge metrics with optional labels.
// It mirrors GaugeBuilder and shares its sentinel errors.
type Float64GaugeBuilder struct {
	factory *MetricsFactory
...
// WithLabels adds labels/attributes to the gauge metric.
// Returns a nil-safe builder if the receiver is nil.
func (g *Float64GaugeBuilder) WithLabels(labels map[string]string) *Float64GaugeBuilder {
	if g == nil {
		return nil
	}

	builder := &Float64GaugeBuilder{
		factory: g.factory,
		gauge:   g.gauge,
		name:    g.name,
		attrs:   make([]attribute.KeyValue, 0, len(g.attrs)+len(labels)),
...
// WithAttributes adds OpenTelemetry attributes to the gauge metric.
// Returns a nil-safe builder if the receiver is nil.
func (g *Float64GaugeBuilder) WithAttributes(attrs ...attribute.KeyValue) *Float64GaugeBuilder {
	if g == nil {
		return nil
	}
...
// idiomatic for instantaneous state (e.g., queue length, in-flight operations).
// It uses only the builder attributes to avoid high-cardinality labels.
func (g *Float64GaugeBuilder) Set(ctx context.Context, value float64) error {
	if g == nil {
		return ErrNilGaugeBuilder
	}
...
// Float64HistogramBuilder provides a fluent API for recording float64 histogram metrics with optional labels.
// It mirrors HistogramBuilder and shares its sentinel errors.
type Float64HistogramBuilder struct {
	factory   *MetricsFactory
...
// WithLabels adds labels/attributes to the histogram metric.
// Returns a nil-safe builder if the receiver is nil.
func (h *Float64HistogramBuilder) WithLabels(labels map[string]string) *Float64HistogramBuilder {
	if h == nil {
		return nil
	}

	builder := &Float64HistogramBuilder{
		factory:   h.factory,
		histogram: h.histogram,
		name:      h.name,
		attrs:     make([]attribute.KeyValue, 0, len(h.attrs)+len(labels)),
...
// WithAttributes adds OpenTelemetry attributes to the histogram metric.
// Returns a nil-safe builder if the receiver is nil.
func (h *Float64HistogramBuilder) WithAttributes(attrs ...attribute.KeyValue) *Float64HistogramBuilder {
	if h == nil {
		return nil
	}
...
// Record records a histogram value
func (h *Float64HistogramBuilder) Record(ctx context.Context, value float64) error {
	if h == nil {
		return ErrNilHistogramBuilder
	}
...
// isFinite reports whether v can be aggregated: NaN and +/-Inf cannot, and a
// single one of them corrupts the aggregate for the rest of the process.
