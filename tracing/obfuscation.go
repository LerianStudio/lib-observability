...
import (
	...
	cn "github.com/LerianStudio/lib-observability/v2/constants"
...
func NewDefaultRedactor() *Redactor {
	fields := redaction.DefaultSensitiveFields()
...
func (r *Redactor) actionFor(path, fieldName string) (RedactionAction, bool) {
	...
	for i := range r.rules {
		...
		if rule.fieldRegex != nil {
			...
			fieldMatch = redaction.IsSensitiveField(fieldName)
		}
...
func obfuscateStructFields(data any, path string, r *Redactor) any {
	...
	case map[string]any:
		...
		for key, value := range v {
			...
			if r != nil {
				redacted, drop, applied := r.redactValue(childPath, key, value)
				if drop {
					continue
				}
			...
			result[key] = obfuscateStructFields(value, childPath, r)
		}
	...
	case []any:
		...
		for i, item := range v {
			...
			result[i] = obfuscateStructFields(item, childPath, r)
		}
...
func ObfuscateStruct(valueStruct any, r *Redactor) (any, error) {
	if valueStruct == nil || r == nil {
	...
	return obfuscateStructFields(data, "", r), nil
}
