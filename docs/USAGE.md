...
Agregue sempre por `tenant_id`. Use `tenant_slug` apenas como legenda
(`{{tenant_slug}}`) e nunca como chave isolada: o slug pode mudar ou ser
...
- **`url.path` é o TEMPLATE da rota, nunca o path concreto (garantia, sem opt-out).** Mesma resolução do middleware Fiber (§2): a rota sai de `r.Pattern` depois que o handler retorna. `/users/42` é exportado como `url.path` = `http.route` = `/users/{id}`; tráfego que não casou rota nenhuma vira `url.path` = `/{unmatched}`, com `http.route` ausente (exigência do OpenTelemetry). Um id, um CPF ou um número de conta no path de entrada NUNCA sai do processo, e um scanner não infla a cardinalidade do atributo.
- **`WithoutCallerAttributes()` (opt-in) tira o CHAMADOR do span:** sem `user_agent.original`, sem `client.address`, sem `network.peer.address` / `network.peer.port`. **Desligado por padrão** — ligue em listener PÚBLICO, onde esses quatro vêm de valores que um chamador não autenticado escolhe (`User-Agent`, `X-Forwarded-For`, endereço do peer) num span que também carrega o id do usuário e do tenant autenticados. O **handler abaixo não é afetado**: recebe o `User-Agent`, o `X-Forwarded-For` e o `RemoteAddr` reais, então access log, rate limiter e allowlist de IP enxergam o que chegou de fato. `server.address` (o host do PRÓPRIO serviço) é mantido nos dois casos.
