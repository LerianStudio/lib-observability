...
3. **Cardinalidade é calculada, não estimada.** Cada combinação de atributos custa 1 série num counter e `explicit_boundary_count + 3` num histograma Prometheus (boundaries explícitos, bucket `+Inf` implícito, `_sum`, `_count`; hoje 14 + 3 = 17). O SDK OTel usa por padrão 2.000 attribute-sets por instrumento e colapsa o excedente em `otel.metric.overflow=true`, descartando a identidade do tenant. O limite efetivo é configurável por `NewTelemetryWithOptions(cfg, WithMetricCardinalityLimit(limit))` (zero = default 2.000 do SDK). O orçamento deve ser calculado separadamente por instrumento contra o limite efetivo do provider adotado. `tenant.name` é funcionalmente 1:1 com `tenant.id`, portanto não multiplica os attribute-sets em estado estável. Um rename cria outro set para cada combinação ativa; com a temporalidade cumulativa padrão, o set antigo pode permanecer no estado do SDK até reiniciar o MeterProvider ou o processo, e a série no backend permanece pelo período de retenção. No cenário semântico estável testado de 50 tenants × 30 rotas: request counter = 1.500 sets; 4xx response counter = 500; 5xx response counter = 500; latency histogram = 150 sets, sem overflow antes de qualquer sobreposição retida por rename. Um único counter com tenant × rota × status teria 9.000 sets e já quebraria o filtro por tenant sob o limite padrão. O SDK reserva um attribute-set para overflow. Para counters, dimensione `rotas normalizadas × (tenants + versões de nome retidas) <= limite efetivo - 1`. Com o limite padrão, 30 rotas e 50 tenants deixam espaço para no máximo 16 versões antigas de nome com atividade em todas as rotas; a 17ª pode gerar overflow. A rota fallback estável para tráfego não casado também conta como rota normalizada. O orçamento não é garantia universal: cada adotante deve recalcular tenants, rotas e histórico de renames esperado durante a vida do processo. Identidade entra em counter dimensionado ou trace, nunca em histograma de alta dimensão.
...
5. **tenant.id e tenant.name** nunca entram em métricas HTTP padrão nem em métricas de infra (`sqlobs`, `redisobs`, `messagingobs`): esses recursos são compartilhados entre tenants e os labels seriam semanticamente errados. Para HTTP, são permitidos somente nas métricas `lerian.*.by_tenant` opt-in, a partir de identidade autenticada explicitamente no contexto da aplicação; header, baggage, metadata, AttrBag e atributos genéricos de span não são fontes confiáveis e não podem fornecer nem sobrescrever a identidade autenticada. `tenant.id` é a chave estável de agregação. `tenant.name` é apenas rótulo mutável de exibição; nunca agregue somente por nome.
...
| `lerian.http.server.requests.by_tenant` | Counter | {request} | tenant.id autenticado, tenant.name opcional, http.route | OPT-IN |
| `lerian.http.server.responses_4xx.by_tenant` | Counter | {request} | tenant.id autenticado, tenant.name opcional, http.route | OPT-IN; somente HTTP 4xx; sem status exato |
| `lerian.http.server.responses_5xx.by_tenant` | Counter | {request} | tenant.id autenticado, tenant.name opcional, http.route | OPT-IN; somente HTTP 5xx; sem status exato |
| `lerian.http.server.latency.by_tenant` | Histogram | s | tenant.id autenticado, tenant.name opcional, http.response.status_class (1xx–5xx, other) | OPT-IN |
