...
3. **Cardinalidade é calculada, não estimada.** Cada combinação de atributos custa 1 série num counter e `explicit_boundary_count + 3` num histograma Prometheus (boundaries explícitos, bucket `+Inf` implícito, `_sum`, `_count`; hoje 14 + 3 = 17). O SDK OTel usa por padrão 2.000 attribute-sets por instrumento e colapsa o excedente em `otel.metric.overflow=true`, descartando a identidade do tenant. O limite efetivo é configurável por `NewTelemetryWithOptions(cfg, WithMetricCardinalityLimit(limit))` (zero = default 2.000 do SDK). Dimensione-o contra o crescimento projetado — aproximadamente `2 × (tenants × rotas normalizadas)` — em vez da contagem de tenants de hoje. O orçamento deve ser calculado separadamente por instrumento contra o limite efetivo do provider adotado. `tenant.name` é funcionalmente 1:1 com `tenant.id`, portanto não multiplica os attribute-sets em estado estável; um rename cria temporariamente um set adicional para cada combinação ativa até a série antiga expirar. No cenário semântico testado de 50 tenants × 30 rotas: request counter = 1.500 sets; 4xx response counter = 500; 5xx response counter = 500; latency histogram = 150 sets, todos sem overflow. No pior caso, cada response counter pode chegar a 1.500 sets e a latência a 300 sets para 6 classes. Um único counter com tenant × rota × status teria 9.000 sets e já quebraria o filtro por tenant sob o limite padrão. O SDK reserva um attribute-set para overflow. Para counters com tenant × rota, o teto teórico de tenants é `floor((limite efetivo - 1) / rotas normalizadas)`; com o limite padrão e 30 rotas, são 66 tenants. A rota fallback estável para tráfego não casado também conta como rota normalizada. O orçamento não é garantia universal: cada adotante deve recalcular tenants autenticados × rotas normalizadas e manter cada instrumento abaixo do limite efetivo. Identidade entra em counter dimensionado ou trace, nunca em histograma de alta dimensão.
