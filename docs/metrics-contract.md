...
| `lerian.http.server.responses_4xx.by_tenant` | Counter | {request} | tenant.id autenticado, http.route | OPT-IN; somente HTTP 4xx; sem status exato |
| `lerian.http.server.responses_5xx.by_tenant` | Counter | {request} | tenant.id autenticado, http.route | OPT-IN; somente HTTP 5xx; sem status exato |
